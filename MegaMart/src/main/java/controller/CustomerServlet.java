...
public class CustomerServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
			...
			if(secret.equals("addCustomer")) {
				...
				List<User> listCustomer= cd.getAllCustomer();
				Gson gson = new GsonBuilder().create();
		        String json = gson.toJson(listCustomer);
		        pw.print(json);
			}
			else if(secret.equals("blockCustomer")) {
				int uId=Integer.parseInt(request.getParameter("id"));
				int status=cd.blockCustomer(uId);
			    if(status>0) {
			    	pw.print("Yes");
			    }else {
			    	pw.print("No");
			...
			else if(secret.equals("getCustomerDataById")) {
				int uId=Integer.parseInt(request.getParameter("id"));
				User user=cd.getCustomerById(uId);
				Gson gson = new GsonBuilder().create();
